-- =====================================================
-- CORRECTED SUPABASE SETUP FOR SAPTAGIRI EXAMS
-- This version avoids system table modifications
...
-- Admins can access everything (using a different approach to avoid recursion)
...
-- Special policy for admin_users table to avoid recursion
CREATE POLICY "Admin users can manage admin_users" ON admin_users FOR ALL USING (
  -- Allow access if user is already an admin OR if no admin exists yet (for initial setup)
  EXISTS (SELECT 1 FROM admin_users WHERE user_id = auth.uid())
  OR 
